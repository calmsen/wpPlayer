...
a.media.getAttribute("autoplay")!==null){a.media.load();a.media.play()}}else if(c.isAndroid){if(a.isVideo){if(a.$media.find("source").length>0)a.media.src=a.$media.find('source[src$="mp4"]').attr("src");a.$media.click(function(){a.media.play()})}}else{a.$media.removeAttr("controls");a.id="mep_"+mejs.mepIndex++;a.container=f('<div id="'+a.id+'" class="mejs-container"><div class="mejs-inner"><div class="mejs-mediaelement"></div><div class="mejs-layers"></div><div class="mejs-divcontrols"><div class="mejs-contcontrols"><div class="mejs-controls"></div></div></div><div class="mejs-clear"></div></div></div>').addClass(a.$media[0].className).insertBefore(a.$media);
a.container.find(".mejs-mediaelement").append(a.$media);a.controls=a.container.find(".mejs-controls");a.layers=a.container.find(".mejs-layers");if(a.isVideo){
a.width=a.$media[0].getAttribute("width") || (a.options.videoWidth>0?a.options.videoWidth:a.options.defaultVideoWidth);
a.height=a.$media[0].getAttribute("height") || (a.options.videoHeight>0?a.options.videoHeight:a.options.defaultVideoHeight);

/*if(cs.init_player_by_css){
	if(a.$media[0].getAttribute("width")===null){
		a.$media[0].setAttribute("width",a.$media.css('width'));
		a.width = a.$media.attr("width");
	}
	if(a.$media[0].getAttribute("height")===null){
		a.$media[0].setAttribute("height",a.$media.css('height'));
		a.height = a.$media.attr("height")
	}
}*/
a.$media.width(a.width).height(a.height);
...
0&&b.domNode.getAttribute("height")===null&&!isNaN(h.target.videoHeight)){b.setPlayerSize(h.target.videoWidth,h.target.videoHeight);b.setControlsSize();b.media.setVideoSize(h.target.videoWidth,h.target.videoHeight)}},false)}b.media.addEventListener("ended",function(){b.media.setCurrentTime(0);b.media.pause();b.setProgressRail&&b.setProgressRail();b.setCurrentRail&&b.setCurrentRail();b.options.loop?b.media.play():b.controls.css("visibility","visible");
/*
var self = b.controller;
self.is_play = false;
if(!self.isFullScreen && self.$cont_player.is('.mejs_1')){
	self.alert(self.isFullScreen);
	self.$cont_player.removeClass('mejs_1').addClass('mejs_0').addClass('mejs-hidden');
	self.view = '';
	self.type_init = '';
	controller_players.$but_play = false;
	controller_players.me_player = false;
	controller_players.$cont_player = false;
	controller_players.idblog = false;
}
*/
},true);b.media.addEventListener("loadedmetadata",
...
(function(f){MediaElementPlayer.prototype.buildplaypause=function(a,c,b,d){var e=f('<div title="Играть" class="mejs-button mejs-playpause-button mejs-play"><button type="button"></button></div>').appendTo(c).click(function(g){g.preventDefault();d.paused?d.play():d.pause();return false});d.addEventListener("play",function(){e.removeClass("mejs-play").addClass("mejs-pause");
},false);d.addEventListener("playing",function(){e.removeClass("mejs-play").addClass("mejs-pause")},false);d.addEventListener("pause",
function(){e.removeClass("mejs-pause").addClass("mejs-play")
...
(function(f){MediaElementPlayer.prototype.buildvolume=function(a,c,b,d){var e=f('<div class="mejs-button mejs-volume-button mejs-mute volume"><button type="button"></button><div class="mejs-volume-slider"><div class="mejs-volume-total"></div><div class="mejs-volume-current"></div><div class="mejs-volume-handle"></div></div></div>').appendTo(c);c=e.find(".mejs-volume-slider");var g=e.find(".mejs-volume-total"),h=e.find(".mejs-volume-current"),j=e.find(".mejs-volume-handle"),l=function(i){i=g.height()-g.height()*
i;j.css("top",i-j.height()/2);h.height(g.height()-i+parseInt(g.css("top").replace(/px/,""),10));h.css("top",i)},m=function(i){var k=g.height(),n=g.offset(),q=parseInt(g.css("top").replace(/px/,""),10);i=i.pageY-n.top;if(i<0)i=0;else if(i>k)i=k;n=(k-i)/k;j.css("top",i-j.height()/2+q);h.height(k-i);h.css("top",i+q);if(n==0){d.setMuted(true);e.removeClass("mejs-mute").addClass("mejs-unmute")}else{d.setMuted(false);e.removeClass("mejs-unmute").addClass("mejs-mute")}n=Math.max(0,n);n=Math.min(n,1);d.setVolume(n)},
...
/**/
if(!$.browser.msie){
(function(f){MediaElementPlayer.prototype.buildfullscreen=function(a,c,b,d){if(a.isVideo){var e=0,g=0,h=a.container,j=f('<div title="Полный экран" class="mejs-button mejs-fullscreen-button"><button type="button"></button></div>').appendTo(c).mousedown(function(){l(mejs.MediaFeatures.hasNativeFullScreen?!d.webkitDisplayingFullscreen:!d.isFullScreen)}),l=function(m){switch(d.pluginType){case "flash":case "silverlight":d.setFullscreen(m);break;case "native":if(mejs.MediaFeatures.hasNativeFullScreen)if(m){d.webkitEnterFullScreen();
...
/*var self = a.controller;
for(var i = 0;i < controller_players.size;i++){	
	var
		player = controller_players.players[i]
	;
	if(player != self){
		if(!player.$cont_player.is('.mejs_0')){
			player.$cont_player.addClass('mejs-hidden');
		}
	}
}
if(self.view == "new"){
	self.$cont_player.removeClass('mejs_2').addClass('mejs_1');
}else
if(self.view == "art"){
	self.$cont_player.removeClass('mejs_3').addClass('mejs_1');
}
$("body").css({'overflow-y':'hidden'});
self.isFullScreen = true;
controller_players.isFullScreen = true;
controller_players.$but_fullscreen = self.$but_fullscreen;
*/
var self = a.controller;
self.$cont_player.removeClass('mejs_3').addClass('mejs_1')
.find('div.mejs-controls').css({'margin-left':''});
d.isFullScreen=true}else{h.removeClass("mejs-container-fullscreen").width(g).height(e).css("z-index",1);a.$media.width(g).height(e);
b.children("div").width(g).height(e);j.removeClass("mejs-unfullscreen").addClass("mejs-fullscreen");a.setControlsSize();
/*var self = a.controller;
for(var i = 0;i < controller_players.size;i++){	
	var
		player = controller_players.players[i]
	...
	if(player != self){
		if(!player.$cont_player.is('.mejs_0')){
			player.$cont_player.removeClass('mejs-hidden');
		}
	}
}
if(self.view == "new"){
	self.$cont_player.removeClass('mejs_1').addClass('mejs_2');
}else
if(self.view == "art"){
	self.$cont_player.removeClass('mejs_1').addClass('mejs_3');
}
$("body").css({'overflow-y':'auto'});
self.isFullScreen = false;
controller_players.isFullScreen = false;
controller_players.$but_fullscreen = self.$but_fullscreen;
*/
var self = a.controller;
self.$cont_player.removeClass('mejs_1').addClass('mejs_3')
.find('div.mejs-controls').css({'margin-left':self.margin_mejs_controls});
d.isFullScreen=false}}};f(document).bind("keydown",function(m){d.isFullScreen&&m.keyCode==27&&l(false)})}}})(jQuery);
}else{
	(function(f){MediaElementPlayer.prototype.buildfullscreen=function(a,c,b,d){
		if(a.isVideo){
			var e=0,g=0,h=a.container,j=f('<div title="Полный экран" class="mejs-button mejs-fullscreen-button"><button type="button"></button></div>').appendTo(c).mousedown(function(){
			l(mejs.MediaFeatures.hasNativeFullScreen?!d.webkitDisplayingFullscreen:!d.isFullScreen)}),
			l=function(m){
			switch(d.pluginType){
				case "flash":
				case "silverlight":
					d.setFullscreen(m);break;
				case "native":
				...
				if(m){
					...
					a.setControlsSize();
					if(a.controller.view == "new"){
						a.controller.$cont_player.removeClass('mejs_2').addClass('mejs_1');
					}else
					if(a.controller.view == "art"){
						a.controller.$cont_player.removeClass('mejs_3').addClass('mejs_1');
					}
					d.isFullScreen=true
				}else{
					...
					a.setControlsSize();
					if(a.controller.view == "new"){
						a.controller.$cont_player.removeClass('mejs_1').addClass('mejs_2');
					}else
					if(a.controller.view == "art"){
						a.controller.$cont_player.removeClass('mejs_1').addClass('mejs_3');
					}
					d.isFullScreen=false
				}
...
(function($) {
    MediaElementPlayer.prototype.integrate_controller = function(self) {
        this.controller = self;
    }
	/*
	MediaElementPlayer.prototype.buildahead = function(player, controls, layers, media) {
        var ahead =  $('<div title="Следующая" style="width:0px;padding:0px;margin:0px;" class="mejs-button mejs-ahead-button"><button style="width:0px;padding:0px;margin:0px;"></button></div>')
            .appendTo(controls)
            .click(function() {
            next_play();
            });     
    ...
    MediaElementPlayer.prototype.buildback = function(player, controls, layers, media) {
        var back =  $('<div title="Предыдущая" style="width:0px;padding:0px;margin:0px;" class="mejs-button mejs-back-button"><button style="width:0px;padding:0px;margin:0px;"></button></div>')
            .appendTo(controls)
            .click(function() {
            prev_play();
            });     
	...
	MediaElementPlayer.prototype.buildplaylist = function(player, controls, layers, media) {
        var playlist =  $('<div title="Playlist" style="width:0px;padding:0px;margin:0px;" class="mejs-button mejs-playlist-button"><button style="width:0px;padding:0px;margin:0px;"></button></div>')
            .appendTo(controls)
            .click(function() {
			$("div.cont_playlist").toggleClass("hide_playlist");
            });     
    }
	
	MediaElementPlayer.prototype.buildvideo = function(player, controls, layers, media) {
        var video =  $('<div title="Раскрыть видео" style="width:0px;padding:0px;margin:0px;" class="mejs-button mejs-video-button"><button style="width:0px;padding:0px;margin:0px;"></button></div>')
            .appendTo(controls)
            .click(function() {
	...
	MediaElementPlayer.prototype.buildresume = function(player, controls, layers, media) {
        var resume =  $('<div style="width:0px;padding:0px;margin:0px;" class="resumeg mejs-button mejs-resume-button "><div class="compl"><a title="Оставить комментарий" href="javascript:void(0);" class="sign n_1" onclick=""></a><span></span></div><div class="readpl"><a title="читать статью" href="javascript:void(0);"  class="sign n_1" onclick=""></a><span></span></div><div class="resppl"><a title="оставить отзыв" href="javascript:void(0);" class="sign n_1" onclick=""></a><span></span></div></div>')
			.appendTo(controls)
            .click(function() {
            });     
    }
	MediaElementPlayer.prototype.buildloadmp4 = function(player, controls, layers, media) {
        var loadmp4 =  $('<div title="Скачать" style="width:0px;padding:0px;margin:0px;" class="mejs-button mejs-loadmp4-button "><a title="Скачать" href="http://newversiya.loc/library/myphp/download.php?filename=' + mp4 + '" class="loadmp4"></a></div>')
			.appendTo(controls)
            .click(function() {
            });     
    }
	MediaElementPlayer.prototype.builddohide = function(player, controls, layers, media) {
        var dohide =  $('<div title="Скрывать" style="width:0px;padding:0px;margin:0px;" class="mejs-button mejs-dohide-button mejs-dohide"><button style="width:0px;padding:0px;margin:0px;"></button></div>')
            .appendTo(controls)
            .click(function() {
            	if($(this).is(".mejs-dohide")){
            		$("div.mejs-dohide-button").attr({'title':'Не скрывать'}).removeClass("mejs-dohide").addClass("mejs-dovisible");	
            	}else
        		if($(this).is(".mejs-dovisible")){
            		$("div.mejs-dohide-button").attr({'title':'Скрывать'}).removeClass("mejs-dovisible").addClass("mejs-dohide");	
            	}
            });     
    }
	*/
	MediaElementPlayer.prototype.buildvolume_1 = function(a,c,b,d){
		var volume_1 = $('<div title="Звук" style="width:0px;padding:0px;margin:0px;" class="mejs-button mejs-volume-button mejs-mute volume_1"><div class="mejs-volume-slider"><div class="mejs-volume-total"></div><div class="mejs-volume-current"></div><div class="mejs-volume-handle"></div></div><button type="button"></button></div>').appendTo(c);
		c = volume_1.find(".mejs-volume-slider");
		var g = volume_1.find(".mejs-volume-total"),
		h = volume_1.find(".mejs-volume-current"),
		j = volume_1.find(".mejs-volume-handle"),
		l = function(i){
			//i = g.width() * i;
			i = 115 * i;
			j.css("left",i - j.width() / 2);
			h.width(i);
		},
		m  = function(i){
			var k = g.width(),
			...
			if(i < 0)i = 0;else if(i > k)i = k;
			n = i/k;
			j.css("left",i - j.width()/2);h.width(i);
			if(n == 0){
			d.setMuted(true);
			volume_1.removeClass("mejs-mute").addClass("mejs-unmute")
			}else{
			d.setMuted(false);
			volume_1.removeClass("mejs-unmute").addClass("mejs-mute")
			}
			n = Math.max(0,n);
			n = Math.min(n,1);
			d.setVolume(n)
		...
		volume_1.find("button").click(function(){
			if(d.muted){
				d.setMuted(false);
				volume_1.removeClass("mejs-unmute").addClass("mejs-mute");
				l(1)
